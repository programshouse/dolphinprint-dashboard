...
const FileUpload = ({
  ...
  // build preview from `value` (File or URL string)
  useEffect(() => {
    let url = null;
  ...
  const emit = (fileOrNull) => {
    // ✅ send both "files" and "value" so ANY parent handler works
    const fakeEvent = {
      target: {
        name,
        value: fileOrNull, // BlogFormTiny reads this
        files: fileOrNull ? [fileOrNull] : [], // WorkshopForm can read this
      },
    };
    onChange?.(fakeEvent);
  };
  ...
  const handleFileChange = (e) => {
    ...
    // revoke this created url when replaced/removed (avoid memory leak)
    // (we also revoke in effect when `value` is File, this is extra safe)
